...
void GOMR2_ObliviousMultiplexer() {
    ...
    bipartiteGraphWeightsGeneration(bipartite_map_glb, weights_glb, numOfTransactions, OMRthreeM, repeatition_glb, seed_glb);
    time_start = chrono::high_resolution_clock::now();
    auto res = receiverDecodingOMR3(lhs_multi_ctr[0], bipartite_map[0], rhs_multi[0], poly_modulus_degree,
                                    secret_key, context, numOfTransactions, party_size_glb, acc_slots+1);
    time_end = chrono::high_resolution_clock::now();
    time_diff = chrono::duration_cast<chrono::microseconds>(time_end - time_start);
    cout << "\nRecipient running time: " << time_diff.count() << "us." << "\n";
...
void GOMR1_ObliviousMultiplexer_BFV() {
    ...
    chrono::microseconds time_diff;
    time_start = chrono::high_resolution_clock::now();
    uint64_t total_plain_ntt = 0, total_load = 0;
    ...
    for(int i = first; i < last; i++) {
        ...
        while(j < numOfTransactions/numcores/poly_modulus_degree){
            ...
            // divide messages into parties, for partySize ciphertexts, each ciphertext p encrypt the PVs of the p-th messages in all groups
            // sum up all ciphertexts into one, s.t. each slot in the final ciphertext encrypts a single group
            packedSICfromPhase1[i][j] = serverOperations1obtainPackedSICWithCluePoly(switchingKey, relin_keys, gal_keys, poly_modulus_degree, context,
                                                                                     params, poly_modulus_degree, &total_plain_ntt, &total_load);
            j++;
            counter[i] += poly_modulus_degree;
            SICPVW_multicore[i].clear();
        }
...
void GOMR1_FG() {
    ...
    auto secret_sk = PVWGenerateSecretKey(params);
    auto shared_sk = mre::MREGenerateSharedSK(params);
    cout << "Finishing generating pk for targeted recipient group\n";

    // step 2. prepare transactions
    vector<int> pertinentMsgIndices;
    auto expected = preparingMREGroupClue(pertinentMsgIndices, numOfTransactions, num_of_pertinent_msgs_glb, params, secret_sk, shared_sk, mreseed);
    ...
    fgomr::FixedGroupDetectionKey switchingKey = fgomr::generateDetectionKey(context, poly_modulus_degree, public_key, secret_key, secret_sk, shared_sk, params);
    Ciphertext packedSIC;
...
void GOMR2_FG() {
    ...
    // step 2. prepare transactions
    vector<int> pertinentMsgIndices;
    auto expected = preparingMREGroupClue(pertinentMsgIndices, numOfTransactions, num_of_pertinent_msgs_glb,
                                          params, secret_sk, shared_sk, mreseed);
    cout << expected.size() << " pertinent msg: Finishing preparing messages with indices: " << pertinentMsgIndices << endl;
    ...
    for (int i = first; i < last; i++) {
        ...
        while (j < numOfTransactions/numcores/poly_modulus_degree) {
            ...
            serverOperations3therest(templhsctr, bipartite_map[i], temprhs, packedSICfromPhase1[i][j], payload_multicore[i],
                            relin_keys, gal_keys_next, public_key_last, poly_modulus_degree, context_next, context_last,
                            params, numOfTransactions_glb, counter[i], number_of_ct, party_size_glb, acc_slots+1);
